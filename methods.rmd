...
author: 'Alice Ledda, Fan Grayson, Toby Nonnenmacher, Leo Loman, UK Health Security Agency'
date: ""
output:
  html_document:
    highlight: kate
...
This document provides information on the code and analysis used for our research project.
...
- epi data that came with the files:
  ...
  - __Deduplicated file "finalEpiDataRef.csv"__
...
- full aligment of all the reconstructed plasmids.
...
New R scripts that run the analysis on deduplicated files:

- script for making all the plots of epidemiological features of dataset (Figure 1):
...
- script for redoing the tree (see forward for details):
  - `scripts/Re-alignment4betterTree.R` and `mask_mutations_mainclade_OXA48.R`
- script for within patient genetic distance analysis:
  - `scripts/withinPatDistNewDedu.R`
- script to create the same patient trees:
  - `scripts/treeSamePAT.R`
...
All of this is in a file called __*`deduplicatedAssembly.csv`*__.
...
The statistical analysis of the distribution of SNPs comparing between patient and within patient population has been done in the following notebooks in the repository:
<!-- [https://gitlab.phe.gov.uk/advanced-analytics/plasmids](https://gitlab.phe.gov.uk/advanced-analytics/plasmids): -->

- `negative_binomial_GLM.R`
- `non_parametric_tests.ipynb`
